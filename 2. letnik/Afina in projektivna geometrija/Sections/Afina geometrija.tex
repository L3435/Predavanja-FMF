...
\begin{definicija}
    \emph{Razsešnost}\index{Razsešnost} afinega prostora $A = a + W$ je \emph{dim A} = dim $U$.
\end{definicija}

\begin{definicija}
    Naj so $a_{i} \in A$ in $\alpha_{i} \in O$ za vse $1 \leq i \leq n$, in naj je $\sum_{i=1}^{n} \alpha_{i} = 1$. Vsoto $\sum_{i=1}^{n} \alpha_{i} a_{i}$ imenujemo \emph{afina kombinacija}\index{Afina kombinacija} točk $a_1,\dots, a_n$.
\end{definicija}

...
\emph{Jedro}\index{Preslikava!Jedro} linearne preslikave $A$ je
\[
\ker A=\set{u\in U\mid Au=0}.
\]
\emph{Slika}\index{Preslikava!Slika} linearne preslikave $A$ je
\[
...
\begin{okvir}
\begin{definicija}
Naj bo $V$ vektorski prostor in naj bo $B=\set{v_1,v_2,\dots,v_n}$ množica vektorjev iz $V$. Pravimo, da je $B$ \emph{baza}\index{Vektorski prostor!Baza} prostora $V$, če
...
%----ERROR 404---- multilinearna algebra

% 1-linearne = linearne
% 2-linearne = bilinearne
%
%
%
%
...
\[
p_B(\lambda)=\det(B-\lambda I)=\det(PAP^{-1}-\lambda PP^{-1})=\det P\cdot \det(A-\lambda I)\cdot P^{-1}=p_A(\lambda).\qedhere
\]
\end{proof}
...
\begin{definicija}
Naj bo $A\colon V\to V$. Pravimo, da se da $A$ \emph{diagonalizirati}\index{Lastna vrednost!Diagonalizacija}, če obstaja baza $\mathcal{B}$ prostora $V$, da je $A_{\mathcal{BB}}$ diagonalna.
...
\begin{definicija}
Naj bo $\F\in\set{\R,\C}$ in $V$ vektorski prostor nad $\F$. \emph{Skalarni produkt}\index{Skalarni produkt} na $V$ je preslikava $\skl{\;,}\colon V\times V\to\F$, ki vektorjema $(u,v)$ priredi skalar $\skl{u,v}$, ki zadošča naslednjim pogojem:
...
\begin{definicija}
Naj bo $V$ normiran prostor in $x,y\in V$. \emph{Razdalja}\index{Vektorski prostor!Razdalja} med vektorjema $x$ in $y$ je
\[
d(x,y)=\norm{x-y}.
\]
\end{definicija}
...
\begin{proof}
Indukcija (\emph{Gram--Schmidtova ortogonalizacija}\index{Gram--Schmidtova ortogonalizacija}).
\end{proof}
...
\begin{definicija}
Naj bosta $V_1$ in $V_2$ vektorska prostora nad $\F$ s skalarnima produktoma $\skl{\;,}_1$ in $\skl{\;,}_2$. \emph{Izomorfizem vektorskih prostorov s skalarnim produktom}\index{Skalarni produkt!Izomorfizem} je preslikava $A\colon V_1\to V_2$, za katero velja:
...
\begin{definicija}
Matrija $A\in\F^{n\times n}$ je \emph{normalna}\index{Matrika!Normalna}, če je $AA^\mathsf{H}=A^\mathsf{H}A$.
\end{definicija}
